        ...
        if isfolder(tDatasetLevel4TrackFolderPath)
            ...
            for j = 1:tDatasetLevel4TrackFolderDirLength
                ...
                if ~strcmp(tDatasetLevel5FolderPhoneName,'.') && ~strcmp(tDatasetLevel5FolderPhoneName,'..') && tDatasetLevel4TrackFolderDir(j).isdir
                    ...
                    AttitudeError = evaluateDataDrivenAttitude(tDatasetLevel5FolderPhonePath);
                    boxplotx = [boxplotx;AttitudeError];

                    seqString = sprintf('%02d', logTrackNumerator);
                    velocityLabel = repmat({seqString},size(AttitudeError,1),1);
...
saveFigFileName = "AttitudeErrorBoxPlot";

        ...
        if isfolder(tDatasetLevel4TrackFolderPath)
            ...
            for j = 1:tDatasetLevel4TrackFolderDirLength
                ...
                if ~strcmp(tDatasetLevel5FolderPhoneName,'.') && ~strcmp(tDatasetLevel5FolderPhoneName,'..') && tDatasetLevel4TrackFolderDir(j).isdir
                    ...
                    tGnssClockBootTime = tGnssClockGnssMeasurementEventTime - tSensorGnssMeasurementRawData(:,2);
                    ...
                    tSequenceGnssClockTable = [tSequenceGnssClockTable;[tSequenceMarker,tGnssClockGnssMeasurementEventTime,tGnssClockBootTime]];

                    % indices = find(tSmartphoneRawSystemClock(:,1) ~= 9);
                    % systemTime = tSmartphoneRawSystemClock(indices,2);
                    % systemClockTime = tSmartphoneRawSystemClock(indices,3);
                    ...
                    % plot(pBootTimeNanosSystemBase*1e-6);
                    % logMsg = sprintf('%f',bootTimeNanosSystemBase(1)*1e-6);
                    % log2terminal('I',TAG,logMsg);
...
pSequenceGnssClockTable = tSequenceGnssClockTable;
gnssClockGnssMeasurementEventTime = pSequenceGnssClockTable(:,2);
gnssClockBootTime = pSequenceGnssClockTable(:,3);

minGnssClockGnssMeasurementEventTime = min(gnssClockGnssMeasurementEventTime);
pReferenceGnssClockTimeGnssMeasurementEventTime = floor(minGnssClockGnssMeasurementEventTime * NS2MS) * MS2NS;
minGnssClockBootTime = min(gnssClockBootTime);
pReferenceGnssClockBootTime = floor(minGnssClockBootTime * NS2MS) * MS2NS;
...
pReferenceSystemTimePosixTimeSecond = rGpsReferencePosixTime + pReferenceGnssClockTimeGnssMeasurementEventTime * NS2S;
pReferenceSystemTimeDatetime = datetime(pReferenceSystemTimePosixTimeSecond,'ConvertFrom','posixtime','TimeZone','Asia/Shanghai');
pReferenceGnssClockBootTimePosixTimeSecond = rGpsReferencePosixTime + pReferenceGnssClockBootTime * NS2S;
pReferenceGnssClockBootTimeDatetime = datetime(pReferenceGnssClockBootTimePosixTimeSecond,'ConvertFrom','posixtime','TimeZone','Asia/Shanghai');
format long;
logMsg = sprintf('绘图参考Gnss Time: %s',string(pReferenceSystemTimeDatetime,'yyyy-MM-dd''T''HH:mm:ss.SSSSSSSSSZZZZZ'));
log2terminal('I',TAG,logMsg);
logMsg = sprintf('绘图参考Boot Time: %s',string(pReferenceGnssClockBootTimeDatetime,'yyyy-MM-dd''T''HH:mm:ss.SSSSSSSSSZZZZZ'));
log2terminal('I',TAG,logMsg);

pX = (gnssClockGnssMeasurementEventTime - pReferenceGnssClockTimeGnssMeasurementEventTime) * NS2S;
pY = (gnssClockBootTime - pReferenceGnssClockBootTime) * NS2MS;
figureObject = figure();
for i = 1:cDatasetLevel4TrackFolderNameListLength
    logTrackNumerator = i;
    tDatasetLevel4TrackFolderName = cDatasetLevel4TrackFolderNameList(i);
    tTrackNumber = str2double(tDatasetLevel4TrackFolderName);
    indices = find(pSequenceGnssClockTable(:,1) == tTrackNumber);
    pTrackX = pX(indices);
    pTrackY = pY(indices);
...
% 网格
if strcmp(cPhone,'HUAWEI_Mate30')
    % axesPropertyYLim = [355 425];
elseif strcmp(cPhone,'GOOGLE_Pixel3')
    % axesPropertyYLim = [0.1 1.4];
end
...
set(get(axesObject,'YLabel'),'String','\fontname{宋体}启动时间\fontname{Times new roman}(ms)');
...
cAndroidSystemClockBootTimeStabilityFileName = strcat('AndroidGnssClockBootTimeStability',cPhoneSplit{1},cPhoneSplit{2},'.png');

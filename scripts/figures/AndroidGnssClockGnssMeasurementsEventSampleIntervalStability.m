...
pY = gnssClockGnssMeasurementEventTime;
figureObject = figure();
for i = 1:cDatasetLevel4TrackFolderNameListLength
    logTrackNumerator = i;
    tDatasetLevel4TrackFolderName = cDatasetLevel4TrackFolderNameList(i);
    ...
    pTrackX = pX(indices);
    pTrackY = pY(indices);
    pTrackDeltaY = pTrackY(2:sampleSize,1) - pTrackY(1:(sampleSize-1),1) - 1 * S2NS;
...
set(get(axesObject,'YLabel'),'String','\fontname{宋体}采样时间间隔\fontname{Times new roman}(ns)');
...
cAndroidSystemClockBootTimeStabilityFileName = strcat('AndroidGnssClockGnssMeasurementsEventSampleInvervalStability',cPhoneSplit{1},cPhoneSplit{2},'.png');

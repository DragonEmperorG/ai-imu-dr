        ...
        if isfolder(tDatasetLevel4TrackFolderPath)
            ...
            for j = 1:tDatasetLevel4TrackFolderDirLength
                ...
                if ~strcmp(tDatasetLevel5FolderPhoneName,'.') && ~strcmp(tDatasetLevel5FolderPhoneName,'..') && tDatasetLevel4TrackFolderDir(j).isdir
                    ...
                    tSmartphoneRawSystemClock = loadSmartphoneRawSystemClock(tDatasetLevel5FolderPhonePath);
                    tSmartphoneRawSystemClockSize = size(tSmartphoneRawSystemClock,1);
                    tSequenceMarker = ones(tSmartphoneRawSystemClockSize,1) * tTrackNumber;
                    tSequenceSystemClockTable = [tSequenceSystemClockTable;[tSequenceMarker,tSmartphoneRawSystemClock]];
...
indices = find(tSequenceSystemClockTable(:,2) ~= 9);
pSequenceSystemClockTable = tSequenceSystemClockTable(indices,:);
systemTime = pSequenceSystemClockTable(:,3);
systemClockTime = pSequenceSystemClockTable(:,4);

minSystemClockTime = min(systemClockTime);
pReferenceSystemClockTime = minSystemClockTime;
bootTimeNanosSystemBase = systemTime * MS2NS - systemClockTime;
minBootTimeNanosSystemBase = min(bootTimeNanosSystemBase);
pReferenceBootTimeNanosSystemBase = floor(minBootTimeNanosSystemBase * NS2MS) * MS2NS;
...
pReferenceSystemTime = systemTime(1) * MS2NS - (systemClockTime(1) - pReferenceSystemClockTime);
pReferenceSystemTimeDatetime = datetime(pReferenceSystemTime * NS2S,'ConvertFrom','posixtime','TimeZone','Asia/Shanghai');
pReferenceBootTimeNanosSystemBaseDatetime = datetime(pReferenceBootTimeNanosSystemBase * NS2S,'ConvertFrom','posixtime','TimeZone','Asia/Shanghai');
format long;
logMsg = sprintf('绘图参考System Time: %s',string(pReferenceSystemTimeDatetime,'yyyy-MM-dd''T''HH:mm:ss.SSSSSSSSSZZZZZ'));
log2terminal('I',TAG,logMsg);
logMsg = sprintf('绘图参考Boot Time: %s',string(pReferenceBootTimeNanosSystemBaseDatetime,'yyyy-MM-dd''T''HH:mm:ss.SSSSSSSSSZZZZZ'));
log2terminal('I',TAG,logMsg);

pX = (systemClockTime - pReferenceSystemClockTime) * NS2S;
pY = (bootTimeNanosSystemBase - pReferenceBootTimeNanosSystemBase) * NS2MS;
figureObject = figure();
for i = 1:cDatasetLevel4TrackFolderNameListLength
    logTrackNumerator = i;
    tDatasetLevel4TrackFolderName = cDatasetLevel4TrackFolderNameList(i);
    tTrackNumber = str2double(tDatasetLevel4TrackFolderName);
    indices = find(pSequenceSystemClockTable(:,1) == tTrackNumber);
    pTrackX = pX(indices);
    pTrackY = pY(indices);
...
cAndroidSystemClockBootTimeStabilityFileName = strcat('AndroidSystemClockBootTimeStability',cPhoneSplit{1},cPhoneSplit{2},'.png');

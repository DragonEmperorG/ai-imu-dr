function [] = plotKITTIComparedTrackPositionCustomDataDriven(folderPath,fileName)
%UNTITLED2 此处显示有关此函数的摘要
%   此处显示详细说明
[~,name,~] = fileparts(fileName);
printFolderPath = folderPath;
saveFigFileName = name + "ComparedTrackPosition";

filterState = loadKittiFilterStateIntegratedCustomDataDriven(folderPath,fileName);
tGroundTruthGeodeticCoordinate = loadKittiGroundTruthGeodeticCoordinate(folderPath);

tGroundTruthLocalCoordinate = convertGeodeticToLocalENUCoordinate(tGroundTruthGeodeticCoordinate(:,2:4));
filterStateNavPosition = getFilterStateNavPosition(filterState);

figureHandle = figure;
referencePosition = tGroundTruthLocalCoordinate;
...
set(filterLineHandle,'DisplayName',"Proposed");
